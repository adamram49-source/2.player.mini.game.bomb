...
function renderBoards() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (gameData.board.player1.includes(i)) cell1.classList.add("bomb");
    if (playerId === "player1" && gameData.status === "choosing") cell1.onclick = () => selectBomb(i);
    else if (playerId === "player2" && gameData.status === "attacking") cell1.onclick = () => attackCell("player1", i);
    player1BoardEl.appendChild(cell1);

    const cell2 = document.createElement("div");
    cell2.classList.add("cell");
    if (gameData.board.player2.includes(i)) cell2.classList.add("bomb");
    if (playerId === "player2" && gameData.status === "choosing") cell2.onclick = () => selectBomb(i);
    else if (playerId === "player1" && gameData.status === "attacking") cell2.onclick = () => attackCell("player2", i);
...
function selectBomb(index) {
  ...
  set(ref(database, `games/${gameCode}/board/${playerId}`), myBombs);
  ...
  if (gameData.board[otherPlayer].length > 0) {
    ...
    set(ref(database, `games/${gameCode}/turn`), playerId);
  }
}

function attackCell(opponent, index) {
  ...
  if (hit) { newHearts[playerId] -= 1; alert("Boom! פצצה!"); }
  else alert("Safe!");
  ...
  const nextTurn = opponent === "player1" ? "player2" : "player1";
